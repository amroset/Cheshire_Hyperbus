...
__attribute__((section(".bulk"), aligned(1024)))
static volatile uint64_t buffer[BUF_BYTES / sizeof(uint64_t)];
...
// ======= Bandwidth kernels =======
uint64_t read(const volatile elem_t *src, int n_el) {
...
uint64_t write(volatile elem_t *dst, int n_el, elem_t v) {
  full_fence(); uint64_t t0 = rdcycle();
...
int main(void) {
  ...
  uart_puts("Boot SEQUENTIAL with BIG LLC\r\n");
  ...
  *reg32(&__base_llc, AXI_LLC_CFG_SPM_HIGH_REG_OFFSET) = 0x0;
  ...
  uint64_t cycles_r = read(buffer, n_el);
