...
void bench_random(int n_el) {
  ...
  *reg32(&__base_llc, AXI_LLC_CFG_SPM_HIGH_REG_OFFSET) = 0xFFFFFFFF;
  ...
  uint64_t cycles_w = 1e10; // dummy value to avoid unused variable warning
  ...
  //uint64_t cycles_r = 1e10; // dummy value to avoid unused variable warning
  ...
  double done_read_dram = perf_read(PERF_COUNTER_DRAM_BASE, PERF_OFF_R_DONE_CNT) - done_write_dram_0;
