...
{
    const size_t elem_mask = ((BUF_BYTES - WINDOW_BYTES) / WINDOW_BYTES); // power-of-two assumption
    uint64_t r = xorshift64star(seed);
    size_t e = (size_t)(r & elem_mask);
    return (e * (WINDOW_BYTES / 8));
}

static inline size_t random_window_offset(uint64_t *seed)
{
    uint64_t r = xorshift64star(seed);
    size_t e = (size_t)(r & (WINDOW_BYTES / 8 - 1));
    return e;
}


// ======= Bandwidth kernels =======
static uint64_t read(const volatile elem_t *src,
                                   uint64_t *seed,
                                   int n_el)
{
    volatile elem_t acc = 0;

    /* choose a base so the window fits */
    size_t base = random_full_offset(seed);
    full_fence();
    uint64_t t0 = rdcycle();

    for (size_t i = 0; i < n_el; i++) {
        size_t off = random_window_offset(seed);
        acc = src[(base + off)];
    }
...
static uint64_t write(volatile elem_t *dst,
                                    uint64_t *seed,
                                    int n_el,
                                    elem_t v)
{
...
void bench_random_window_llc(int n_el) {
  ...
  *reg32(&__base_llc, AXI_LLC_CFG_SPM_HIGH_REG_OFFSET) = 0x0;
  ...
  /*volatile elem_t *buffer = bench_buf_ptr();
  ...
  uint64_t cycles_r = read(buffer, &seed_read, n_el);
